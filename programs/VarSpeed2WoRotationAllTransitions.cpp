...
void build_ocps_table(double v_min, double v_max, double u_max, double xFinalIn, double yFinalIn, int orientFinalIn, int startOrientation, Wind windIn, std::map<long, Candidate>& ocps_table) {
    ...
    for (auto & neighbor : neighbors) {
        for (int startOrient = 0; startOrient < ORIENTATIONS; startOrient++) {
            for (int orientFinal = 0; orientFinal < ORIENTATIONS; orientFinal++) {
                ...
                std::cout << "************************************************" << std::endl;
                std::cout << "************************************************" << std::endl;
                build_ocps_table(v_min, v_max, u_max, neighbor[0], neighbor[1], orientFinal, startOrient, wi, ocps);
                transitions++;
            }
        }
    }
